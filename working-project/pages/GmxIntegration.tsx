...
export default function GmxIntegration() {
  ...
  const address = useMemo(() => {
    ...
    if (authenticated && ready) {
      ...
      const privyWallet = wallets.find((w: { address?: string; walletClientType?: string }) =>
      ...
      const ethereumWallet = wallets.find((w: { address?: string }) => isEthereumAddress(w.address));
  ...
  const isPrivyWallet = authenticated && ready && wallets.some((w: { address?: string; walletClientType?: string }) => w.address === address && w.walletClientType === 'privy');
  ...
  const loadGmxSdk = useCallback(async (): Promise<GmxSdk> => {
    ...
    if (isPrivyWallet) {
      const privyWallet = wallets.find((w: { address?: string; walletClientType?: string }) => w.address === address && w.walletClientType === 'privy');
      if (!privyWallet) {
      ...
      const privyWalletClient = {
        ...
        transport: {
          request: async ({ method, params }: { method: string; params?: unknown[] }) => {
            ...
            return await privyProvider.request({ 
              method, 
              params 
        ...
        writeContract: async ({ 
          ...
          // Convert viem ABI to ethers.js compatible format
          const abiJson = JSON.stringify(abi);
          const ethersAbi = JSON.parse(abiJson) as ethers.InterfaceAbi;
          const contract = new ethers.Contract(contractAddress, ethersAbi, signer);
          const tx = await contract[functionName](...args, { value });
          return tx.hash;
        },
        sendTransaction: async (transaction: { to: `0x${string}`; value?: bigint; data?: `0x${string}`; gas?: bigint; maxFeePerGas?: bigint; maxPriorityFeePerGas?: bigint; gasPrice?: bigint }) => {
