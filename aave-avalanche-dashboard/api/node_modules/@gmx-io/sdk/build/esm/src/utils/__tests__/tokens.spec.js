...
import { parseContractPrice, convertToContractPrice, convertToContractTokenPrices, convertToTokenAmount, convertToUsd, getMidPrice, getIsEquivalentTokens, getTokenData, getTokensRatioByAmounts, getTokensRatioByMinOutputAmountAndTriggerPrice, } from "../tokens";
...
describe("getIsEquivalentTokens", () => {
    it("checks address, wrappedAddress, synthetic, and symbol", () => {
        ...
        expect(getIsEquivalentTokens({ address: "0xA", isSynthetic: true, symbol: "SYN" }, { address: "0xB", isSynthetic: true, symbol: "SYN" })).toBe(true);
...
describe("getTokensRatioByAmounts", () => {
    it("returns ratio of two token amounts", () => {
        ...
        expect(result.ratio).toBe((((1000n * PRECISION) / expandDecimals(1, 2)) * PRECISION) / ((500n * PRECISION) / expandDecimals(1, 2)));
    });
});
