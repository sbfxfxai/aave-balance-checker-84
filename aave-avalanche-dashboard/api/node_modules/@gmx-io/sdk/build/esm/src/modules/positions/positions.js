...
import { hashedPositionKey, MAX_AUTO_CANCEL_ORDERS_KEY, MIN_COLLATERAL_USD_KEY, MIN_POSITION_SIZE_USD_KEY, uiFeeFactorKey, } from "../../configs/dataStore";
import { getPositionFee } from "../../utils/fees";
import { getContractMarketPrices, getMarketIndexName, getMarketPoolName, getMaxAllowedLeverageByMinCollateralFactor, } from "../../utils/markets";
import { basisPointsToFloat, getBasisPoints } from "../../utils/numbers";
import { getByKey } from "../../utils/objects";
import { getEntryPrice, getLeverage, getLiquidationPrice, getNetPriceImpactDeltaUsdForDecrease, getPositionKey, getPositionNetValue, getPositionPendingFeesUsd, getPositionPnlAfterFees, getPositionPnlUsd, } from "../../utils/positions";
...
export class Positions extends Module {
    ...
    async getPositionsInfo(p) {
        ...
        const positionsInfoData = Object.keys(positionsData).reduce((acc, positionKey) => {
            ...
            const pendingFundingFeesUsd = convertToUsd(position.fundingFeeAmount, collateralToken.decimals, collateralToken.prices.minPrice);
            const pendingClaimableFundingFeesLongUsd = convertToUsd(position.claimableLongTokenAmount, marketInfo.longToken.decimals, marketInfo.longToken.prices.minPrice);
            const pendingClaimableFundingFeesShortUsd = convertToUsd(position.claimableShortTokenAmount, marketInfo.shortToken.decimals, marketInfo.shortToken.prices.minPrice);
            ...
            const closeAcceptablePriceInfo = marketInfo
                ...
                : undefined;
            const positionFeeInfo = getPositionFee(marketInfo, position.sizeInUsd, closeAcceptablePriceInfo?.balanceWasImproved ?? false, userReferralInfo, uiFeeFactor);
            ...
            const remainingCollateralAmount = convertToTokenAmount(remainingCollateralUsd, collateralToken.decimals, collateralMinPrice);
            ...
            const pnlPercentage = collateralUsd !== undefined && collateralUsd != 0n ? getBasisPoints(pnl, collateralUsd) : 0n;
            ...
            const pnlAfterFeesPercentage = collateralUsd != 0n ? getBasisPoints(pnlAfterFees, collateralUsd + closingFeeUsd) : 0n;
