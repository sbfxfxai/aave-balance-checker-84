...
export function basisPointsToFloat(basisPoints) {
...
export function numberToBigint(value, decimals) {
...
export function bigintToNumber(value, decimals) {
    const negative = value < 0;
    if (negative)
        value *= -1n;
...
export function formatUsd(usd, opts = {}) {
...
export function formatPercentage(percentage, opts = {}) {
...
export function formatTokenAmount(amount, tokenDecimals, symbol, opts = {}) {
...
export function formatTokenAmountWithUsd(tokenAmount, usdAmount, tokenSymbol, tokenDecimals, opts = {}) {
 ...
 */
export function formatRatePercentage(rate, opts) {
...
export function formatUsdPrice(price, opts = {}) {
    if (price === undefined) {
        return;
    }
    if (price < 0n) {
...
export function formatPercentageDisplay(percentage, hideThreshold) {
    if (hideThreshold && percentage < hideThreshold) {
        return "";
    }
    return `${percentage}%`;
}
export function formatAmountHuman(amount, tokenDecimals, showDollar = false, displayDecimals = 1) {
    if (amount === undefined) {
        return "...";
    }
...
export function formatBalanceAmount(amount, tokenDecimals, tokenSymbol, { showZero = false, toExponential = true, isStable = false, signed = false, } = {}) {
    if (amount === undefined)
        return "-";
    if (amount === 0n) {
        if (showZero === true) {
...
export function numberWithCommas(x, { showDollar = false } = {}) {
...
export const formatAmount = (amount, tokenDecimals, displayDecimals, useCommas, defaultValue, visualMultiplier) => {
...
export const formatKeyAmount = (map, key, tokenDecimals, displayDecimals, useCommas) => {
    ...
    return formatAmount(value, tokenDecimals, displayDecimals, useCommas);
};
...
export function getLimitedDisplay(amount, tokenDecimals, opts = {}) {
...
export function getPlusOrMinusSymbol(value, opts = {}) {
    if (value === undefined) {
        return "";
    }
...
export function roundWithDecimals(value, opts) {
...
export function toBigNumberWithDecimals(value, decimals) {
    if (!value)
        return BN_ZERO;
...
export function roundUpDivision(a, b) {
    return (a + b - 1n) / b;
}
export function roundToTwoDecimals(n) {
    return Math.round(n * 100) / 100;
}
export function roundToOrder(n, significantDigits = 1) {
...
export function roundBigIntToDecimals(value, tokenDecimals, roundToDecimals) {
...
export function minBigNumber(...args) {
    if (!args.length)
        return undefined;
    return args.reduce((acc, num) => (num < acc ? num : acc), args[0]);
}
export function maxbigint(...args) {
    if (!args.length)
        return undefined;
    return args.reduce((acc, num) => (num > acc ? num : acc), args[0]);
}
...
export function serializeBigIntsInObject(obj) {
...
export function deserializeBigIntsInObject(obj) {
    const result = Array.isArray(obj) ? [] : {};
    for (const key in obj) {
        const value = obj[key];
...
export function calculateDisplayDecimals(price, decimals = USD_DECIMALS, visualMultiplier = 1, isStable = false) {
...
export function absDiffBps(value, base) {
