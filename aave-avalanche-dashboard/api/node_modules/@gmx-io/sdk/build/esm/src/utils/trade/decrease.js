...
import { applyFactor, BASIS_POINTS_DIVISOR_BIGINT, expandDecimals, getBasisPoints, roundUpDivision, USD_DECIMALS, MaxUint256, } from "../numbers";
import { getLeverage, getLiquidationPrice, getMinCollateralFactorForPosition, getNetPriceImpactDeltaUsdForDecrease, getPositionPnlUsd, } from "../positions";
import { getAcceptablePriceInfo, getDefaultAcceptablePriceImpactBps, getMarkPrice, getOrderThresholdType, } from "../prices";
import { getSwapStats } from "../swap";
import { convertToTokenAmount, convertToUsd, getIsEquivalentTokens } from "../tokens";
export function getDecreasePositionAmounts(p) {
    ...
    const estimatedCollateralUsd = convertToUsd(position.collateralAmount, collateralToken.decimals, values.collateralPrice);
    ...
    values.payedRemainingCollateralUsd = convertToUsd(payedInfo.paidRemainingCollateralAmount, collateralToken.decimals, values.collateralPrice);
    // Collateral delta
    if (values.isFullClose) {
        ...
        const remainingCollateralUsd = convertToUsd(payedInfo.remainingCollateralAmount, collateralToken.decimals, values.collateralPrice);
...
export function getMinCollateralUsdForLeverage(position, openInterestDelta) {
...
export function estimateCollateralCost(baseUsd, collateralToken, collateralPrice) {
...
export function getTotalFeesUsdForDecrease({ positionFeeUsd, borrowingFeeUsd, fundingFeeUsd, swapProfitFeeUsd, swapUiFeeUsd, uiFeeUsd, pnlUsd, totalPendingImpactDeltaUsd, }) {
