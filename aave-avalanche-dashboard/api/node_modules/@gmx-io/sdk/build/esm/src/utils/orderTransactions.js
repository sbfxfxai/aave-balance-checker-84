...
export function buildSwapOrderPayload(p) {
...
export function buildIncreaseOrderPayload(p) {
    ...
    if (p.acceptablePrice === MaxUint256) {
        ...
        acceptablePrice = convertToContractPrice(applySlippageToPrice(p.allowedSlippage, p.acceptablePrice, true, p.isLong), indexToken.decimals);
    }
    let triggerPrice;
...
export function buildDecreaseOrderPayload(p) {
    const indexToken = getToken(p.chainId, p.indexTokenAddress);
    ...
    if (p.acceptablePrice === MaxUint256) {
        ...
        acceptablePrice = convertToContractPrice(applySlippageToPrice(p.allowedSlippage, p.acceptablePrice, false, p.isLong), indexToken.decimals);
    }
    let triggerPrice;
    if (p.triggerPrice === MaxUint256) {
        triggerPrice = MaxUint256;
...
export function buildTwapOrdersPayloads(p, twapParams) {
...
export function getIsTwapOrderPayload(p) {
    return p.numbers.validFromTime !== 0n;
}
export function buildUpdateOrderPayload(p) {
    const indexToken = getToken(p.chainId, p.indexTokenAddress);
    return {
        params: p,
        updatePayload: {
...
export function getBatchTotalExecutionFee({ batchParams: { createOrderParams, updateOrderParams }, tokensData, chainId, }) {
...
export function getBatchTotalPayCollateralAmount(batchParams) {
    const payAmounts = {};
...
export function getBatchExternalSwapGasLimit(batchParams) {
...
export function buildTokenTransfersParamsForDecrease({ chainId, executionFeeAmount, collateralTokenAddress, collateralDeltaAmount, swapPath, minOutputUsd, receiveTokenAddress, }) {
...
export function buildTokenTransfersParamsForIncreaseOrSwap({ chainId, receiver, payTokenAddress, payTokenAmount, receiveTokenAddress, executionFeeAmount, externalSwapQuote, minOutputAmount, swapPath, }) {
...
export function getBatchExternalCalls(batchParams) {
    const externalCalls = [];
...
export function combineExternalCalls(externalCalls) {
    const sendTokensMap = {};
...
export function getEmptyExternalCallsPayload() {
    return {
        sendTokens: [],
        sendAmounts: [],
        externalCallTargets: [],
...
export function getExternalCallsPayload({ chainId, account, quote, }) {
    ...
    if (quote.needSpenderApproval) {
        ...
        payload.externalCallDataList.push(encodeFunctionData({
            abi: ERC20ABI,
...
export function getBatchOrderMulticallPayload({ params }) {
...
export function buildCreateOrderMulticall(params) {
...
export function buildUpdateOrderMulticall(updateTxn) {
...
export function buildCancelOrderMulticall({ params }) {
...
export function encodeExchangeRouterMulticall(multicall) {
    const encodedMulticall = multicall.map((call) => encodeFunctionData({
...
export function getIsEmptyBatch(orderParams) {
    if (!orderParams) {
        return true;
    }
...
export function getBatchIsNativePayment(orderParams) {
    return orderParams.createOrderParams.some((o) => o.tokenTransfersParams?.isNativePayment);
}
export function getIsInvalidBatchReceiver(batchParams, signerAddress) {
