...
import { getIsMarketAvailableForExpressSwaps } from "../markets";
import { buildMarketsAdjacencyGraph } from "./buildMarketsAdjacencyGraph";
import { createMarketEdgeLiquidityGetter, createNaiveNetworkEstimator, createNaiveSwapEstimator, createSwapEstimator, getBestSwapPath, getMarketAdjacencyGraph, getMaxLiquidityMarketSwapPathFromTokenSwapPaths, getNaiveBestMarketSwapPathsFromTokenSwapPaths, getTokenSwapPathsForTokenPairPrebuilt, marketRouteToMarketEdges, } from "./swapRouting";
import { getSwapPathStats } from "./swapStats";
export const getWrappedAddress = (chainId, address) => {
...
function buildMarketAdjacencyGraph(chainId, disabledMarkets) {
    ...
    const strippedMarkets = Object.fromEntries(Object.entries(MARKETS[chainId]).filter(([marketAddress]) => !disabledMarketAddresses.includes(marketAddress)));
        ...
        if (manualPath !== undefined) {
            ...
            if (naiveEstimator) {
                ...
                if (naiveSwapRoutes?.length) {
                    const edges = naiveSwapRoutes.map((path) => marketRouteToMarketEdges(path, wrappedFromAddress, marketsInfoData));
