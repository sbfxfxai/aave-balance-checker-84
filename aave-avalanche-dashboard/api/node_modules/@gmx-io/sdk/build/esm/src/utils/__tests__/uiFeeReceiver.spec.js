...
import { createTwapUiFeeReceiver, decodeTwapUiFeeReceiver, setUiFeeReceiverIsExpress } from "../twap/uiFeeReceiver";
describe("uiFeeReceiver", () => {
    ...
    describe("createTwapUiFeeReceiver", () => {
        ...
        it("should correctly encode twapId as hex", () => {
            expect(parseInt(decodeTwapUiFeeReceiver(createTwapUiFeeReceiver({ numberOfParts: 10 }))?.twapId ?? "", 16)).not.toBeNaN();
            expect(parseInt(decodeTwapUiFeeReceiver(createTwapUiFeeReceiver({ numberOfParts: 21 }))?.twapId ?? "", 16)).not.toBeNaN();
        });
    });
});
describe("setUiFeeReceiverIsExpress", () => {
    it("should correctly set isExpress for simple uiFeeReceiver", () => {
        expect(setUiFeeReceiverIsExpress("0xff00000000000000000000000000000000000001", true)).toEqual("0xff00000000000000000000000000000100000001");
        expect(setUiFeeReceiverIsExpress("0xff00000000000000000000000000000000000001", false)).toEqual("0xff00000000000000000000000000000000000001");
    });
    it("should correctly set isExpress for twap uiFeeReceiver", () => {
        expect(setUiFeeReceiverIsExpress("0xff0000000000000000000000000000000a123401", true)).toEqual("0xff0000000000000000000000000000010a123401");
        expect(setUiFeeReceiverIsExpress("0xff0000000000000000000000000000000a123401", false)).toEqual("0xff0000000000000000000000000000000a123401");
    });
});
