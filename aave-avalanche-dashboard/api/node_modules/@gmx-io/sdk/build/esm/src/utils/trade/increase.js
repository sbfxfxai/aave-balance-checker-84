...
import { capPositionImpactUsdByMaxImpactPool, capPositionImpactUsdByMaxPriceImpactFactor, getPositionFee, getPriceImpactForPosition, getTotalSwapVolumeFromSwapStats, } from "../fees";
import { applyFactor } from "../numbers";
import { getEntryPrice, getLeverage, getLiquidationPrice, getPositionPnlUsd, getPriceImpactDiffUsd, } from "../positions";
import { getAcceptablePriceInfo, getDefaultAcceptablePriceImpactBps, getMarkPrice, getOrderThresholdType, } from "../prices";
import { getSwapAmountsByFromValue, getSwapAmountsByToValue } from "../swap";
import { convertToTokenAmount, convertToUsd, getIsEquivalentTokens, getTokensRatioByPrice } from "../tokens";
export function getIncreasePositionAmounts(p) {
    ...
    if (strategy === "leverageByCollateral" &&
        ...
        values.initialCollateralUsd = convertToUsd(initialCollateralAmount, initialCollateralToken.decimals, values.initialCollateralPrice);
        ...
        values.sizeDeltaUsd = bigMath.mulDiv(baseCollateralUsd - basePositionFeeInfo.positionFeeUsd - baseUiFeeUsd - values.swapUiFeeUsd, leverage, BASIS_POINTS_DIVISOR_BIGINT);
        ...
        values.collateralDeltaUsd =
            baseCollateralUsd -
                ...
                values.uiFeeUsd -
                values.swapUiFeeUsd;
        values.collateralDeltaAmount = convertToTokenAmount(values.collateralDeltaUsd, collateralToken.decimals, values.collateralPrice);
    }
        ...
        values.initialCollateralUsd = convertToUsd(values.initialCollateralAmount, initialCollateralToken.decimals, values.initialCollateralPrice);
    }
        ...
        if (initialCollateralAmount !== undefined && initialCollateralAmount > 0) {
            ...
            const swapAmountIn = values.swapStrategy.amountIn;
            const baseCollateralUsd = convertToUsd(swapAmountIn, initialCollateralToken.decimals, values.initialCollateralPrice);
            values.collateralDeltaUsd =
                baseCollateralUsd -
                    values.positionFeeUsd -
                    values.borrowingFeeUsd -
...
export function getTokensRatio({ fromToken, toToken, triggerRatioValue, markPrice, }) {
...
export function leverageBySizeValues({ collateralToken, leverage, sizeDeltaUsd, collateralPrice, positionFeeUsd, borrowingFeeUsd, uiFeeUsd, swapUiFeeUsd, fundingFeeUsd, }) {
...
export function getIncreasePositionPrices({ triggerPrice, indexToken, initialCollateralToken, collateralToken, limitOrderType, isLong, }) {
    let indexPrice;
...
export function getNextPositionValuesForIncreaseTrade(p) {
    ...
    if (nextPendingImpactDeltaUsd > 0) {
        nextPendingImpactDeltaUsd = capPositionImpactUsdByMaxPriceImpactFactor(marketInfo, nextSizeUsd, nextPendingImpactDeltaUsd);
    }
