// Execute GMX trade using the SDK (same as frontend)
...
async function executeGmxWithSdk() {
  ...
  console.log('Executing GMX BTC Long with SDK...\n');
  ...
  try {
    ...
    // Override callContract to capture hash and attach execution fee
    const originalCallContract = sdk.callContract.bind(sdk);
    sdk.callContract = async (contractAddress, abi, method, params, opts) => {
      console.log(`[SDK] Calling ${method} on ${contractAddress.slice(0, 10)}...`);
      ...
      const h = await originalCallContract(contractAddress, abi, method, params, opts);
      submittedHash = h;
