...
class Markets extends base_1.Module {
    async getClaimableFundingData() {
        ...
        return this.sdk
            .executeMulticall((0, query_builders_1.buildClaimableFundingDataRequest)({
            chainId,
            account,
            marketsAddresses,
            marketsData,
            ...
            .then((result) => {
            return Object.entries(result.data).reduce((claimableFundingData, [marketAddress, callsResult]) => {
                const market = (0, objects_1.getByKey)(marketsData, marketAddress);
                if (!market) {
                    return claimableFundingData;
                }
                const marketDivisor = (0, markets_1.getMarketDivisor)(market);
    ...
    async getMarketsValues({ marketsAddresses, marketsData, tokensData, }) {
        const dataStoreAddress = (0, contracts_1.getContract)(this.chainId, "DataStore");
        const syntheticsReaderAddress = (0, contracts_1.getContract)(this.chainId, "SyntheticsReader");
        const request = await (0, query_builders_1.buildMarketsValuesRequest)(this.chainId, {
            marketsAddresses,
            marketsData,
            tokensData,
            dataStoreAddress,
        ...
        return this.sdk.executeMulticall(request).then((res) => {
            const result = marketsAddresses.reduce((acc, marketAddress) => {
                ...
                const longInterestUsingLongToken = BigInt(dataStoreValues.longInterestUsingLongToken.returnValues[0]) / marketDivisor;
                const longInterestUsingShortToken = BigInt(dataStoreValues.longInterestUsingShortToken.returnValues[0]) / marketDivisor;
                const shortInterestUsingLongToken = BigInt(dataStoreValues.shortInterestUsingLongToken.returnValues[0]) / marketDivisor;
                const shortInterestUsingShortToken = BigInt(dataStoreValues.shortInterestUsingShortToken.returnValues[0]) / marketDivisor;
                ...
                const longInterestInTokensUsingLongToken = BigInt(dataStoreValues.longInterestInTokensUsingLongToken.returnValues[0]) / marketDivisor;
                const longInterestInTokensUsingShortToken = BigInt(dataStoreValues.longInterestInTokensUsingShortToken.returnValues[0]) / marketDivisor;
                const shortInterestInTokensUsingLongToken = BigInt(dataStoreValues.shortInterestInTokensUsingLongToken.returnValues[0]) / marketDivisor;
                const shortInterestInTokensUsingShortToken = BigInt(dataStoreValues.shortInterestInTokensUsingShortToken.returnValues[0]) / marketDivisor;
    ...
    async getMarketsConfigs({ marketsAddresses, marketsData, }) {
        const dataStoreAddress = (0, contracts_1.getContract)(this.chainId, "DataStore");
        const request = await (0, query_builders_1.buildMarketsConfigsRequest)(this.chainId, {
            marketsData,
            marketsAddresses,
            dataStoreAddress,
        });
        return this.sdk.executeMulticall(request).then((res) => {
            const result = marketsAddresses.reduce((acc, marketAddress) => {
                ...
                acc[marketAddress] = {
                    ...
                    minCollateralFactorForOpenInterestShort: dataStoreValues.minCollateralFactorForOpenInterestShort.returnValues[0],
                    ...
                    positionFeeFactorForBalanceWasNotImproved: dataStoreValues.positionFeeFactorForBalanceWasNotImproved.returnValues[0],
            ...
            dataStore: {
                ...
                calls: {
                    useOpenInterestInTokensForBalance: {
                        methodName: "getBool",
                        params: [dataStore_1.USE_OPEN_INTEREST_IN_TOKENS_FOR_BALANCE],
                    },
                },
            },
        })
    ...
    async getMarkets(offset = 0n, limit = 100n) {
        ...
        const readerAddress = (0, contracts_1.getContract)(this.chainId, "SyntheticsReader");
        const dataStoreAddress = (0, contracts_1.getContract)(this.chainId, "DataStore");
        const apiMarkets = await this.sdk.oracle.getMarkets();
        const configMarkets = this.sdk.config.markets ?? {};
        const marketsMap = apiMarkets.reduce((acc, market) => {
        ...
        const markets = await this.sdk
            ...
            .then((res) => {
            return res.data.markets.markets.returnValues.map((market) => {
                return {
        ...
        const chainId = this.chainId;
        const marketsResult = markets.reduce((acc, market) => {
            try {
                if (!marketsMap[market.marketTokenAddress]?.isListed) {
                    return acc;
                }
                const indexToken = (0, tokens_1.getToken)(chainId, (0, tokens_1.convertTokenAddress)(chainId, market.indexTokenAddress, "native"));
                const longToken = (0, tokens_1.getToken)(chainId, market.longTokenAddress);
                const shortToken = (0, tokens_1.getToken)(chainId, market.shortTokenAddress);
                const isSameCollaterals = market.longTokenAddress === market.shortTokenAddress;
                const isSpotOnly = market.indexTokenAddress === viem_1.zeroAddress;
                const name = (0, markets_1.getMarketFullName)({ indexToken, longToken, shortToken, isSpotOnly });
    ...
    async getMarketsInfo() {
        ...
        for (const marketAddress of marketsAddresses) {
            ...
            const indexToken = market
                ? (0, objects_1.getByKey)(tokensData, (0, tokens_1.convertTokenAddress)(this.chainId, market.indexTokenAddress, "native"))
                : undefined;
    ...
    async getDailyVolumes() {
        ...
        return (0, graphqlFetcher_1.default)(endpoint, POSITIONS_VOLUME_INFOS_QUERY).then((data) => {
