...
import { GAS_PRICE_BUFFER_MAP, GAS_PRICE_PREMIUM_MAP, getViemChain, MAX_FEE_PER_GAS_MAP, MAX_PRIORITY_FEE_PER_GAS_MAP, } from "../configs/chains";
import { BASIS_POINTS_DIVISOR_BIGINT } from "../configs/factors";
import { bigMath } from "./bigmath";
export async function getGasPrice(client, chainId) {
    let maxFeePerGas = MAX_FEE_PER_GAS_MAP[chainId];
    const premium = GAS_PRICE_PREMIUM_MAP[chainId] || 0n;
    const feeData = await withRetry(() => client.estimateFeesPerGas({
        type: "legacy",
    ...
    const bufferBps = GAS_PRICE_BUFFER_MAP[chainId] || 0n;
    const buffer = bigMath.mulDiv(gasPrice, bufferBps, BASIS_POINTS_DIVISOR_BIGINT);
    return {
        gasPrice: gasPrice + buffer + premium,
    };
}
export async function getGasLimit(client, account, contractAddress, abi, method, params = [], value) {
...
export async function callContract(sdk, contractAddress, abi, method, params, opts = {}) {
    const txnOpts = {};
