...
export function getSwapCapacityUsd(marketInfo, isLong) {
...
export function getSwapStats(p) {
    ...
    if (priceImpactDeltaUsd > 0) {
        ...
        const { impactDeltaAmount: negativeImpactAmount } = applySwapImpactWithCap(marketInfo, tokenIn, priceImpactDeltaUsd);
    ...
    const capacityUsd = getSwapCapacityUsd(marketInfo, getTokenPoolType(marketInfo, tokenInAddress) === "long");
    const isOutCapacity = capacityUsd < usdInAfterFees;
    const liquidity = getAvailableUsdLiquidityForCollateral(marketInfo, getTokenPoolType(marketInfo, tokenOutAddress) === "long");
