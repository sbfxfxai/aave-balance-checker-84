...
export function getSyntheticTokens(chainId) {
...
export function isValidTokenSafe(chainId, address) {
    return address in TOKENS_MAP[chainId];
}
...
export function getTokenBySymbol(chainId, symbol, { isSynthetic, version, symbolType = "symbol", } = {}) {
...
export function isChartAvailableForToken(chainId, tokenSymbol) {
    let token;
    try {
...
export function getTokenBySymbolSafe(chainId, symbol, params = {}) {
    try {
...
export function isTokenInList(token, tokenList) {
    return tokenList.some((t) => t.address === token.address);
}
export function isSimilarToken(tokenA, tokenB) {
...
export function isUsdBasedStableToken(token) {
